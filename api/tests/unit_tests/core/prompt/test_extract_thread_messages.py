...
import core.prompt.utils.get_thread_messages_length as length_module
...
class _DatabaseBinding:
    """Expose the real SQLite session used by conversation message queries."""
...
@pytest.fixture
def message_session(sqlite_engine: Engine, monkeypatch: pytest.MonkeyPatch) -> Iterator[Session]:
    """Create only the legacy Message table and bind its scoped query."""

    Message.__table__.create(sqlite_engine)
    with Session(sqlite_engine, expire_on_commit=False) as session:
        monkeypatch.setattr(length_module, "db", _DatabaseBinding(session))
        yield session
