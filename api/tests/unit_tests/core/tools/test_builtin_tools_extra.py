...
def test_current_time_tool():
    ...
    utc_text = list(current_tool.invoke(session=_ORM_SESSION, user_id="u", tool_parameters={"timezone": "UTC"}))[
        0
    ].message.text
    assert utc_text

    invalid_tz = list(
        current_tool.invoke(session=_ORM_SESSION, user_id="u", tool_parameters={"timezone": "Invalid/TZ"})
    )[0].message.text
...
def test_localtime_to_timestamp_tool():
    ...
    ts_message = list(
        localtime_tool.invoke(
            session=_ORM_SESSION, user_id="u", tool_parameters={"localtime": "2024-01-01 10:00:00", "timezone": "UTC"}
        )
    )[0].message.text
...
def test_timestamp_to_localtime_tool():
    ...
    local_text = list(
        to_local_tool.invoke(
            session=_ORM_SESSION, user_id="u", tool_parameters={"timestamp": 1704067200, "timezone": "UTC"}
        )
    )[0].message.text
...
def test_weekday_tool():
    ...
    valid = list(
        weekday_tool.invoke(session=_ORM_SESSION, user_id="u", tool_parameters={"year": 2024, "month": 1, "day": 1})
    )[0].message.text
    ...
    invalid = list(
        weekday_tool.invoke(session=_ORM_SESSION, user_id="u", tool_parameters={"year": 2024, "month": 2, "day": 31})
    )[0].message.text
    assert "Invalid date" in invalid
    with pytest.raises(ValueError, match="Month is required"):
        list(weekday_tool.invoke(session=_ORM_SESSION, user_id="u", tool_parameters={"year": 2024, "day": 1}))
...
def test_simple_code_invalid_language():
    ...
    with pytest.raises(ValueError, match="Only python3 and javascript"):
        list(
            simple_code.invoke(
                session=_ORM_SESSION, user_id="u", tool_parameters={"language": "go", "code": "fmt.Println(1)"}
            )
        )
...
def test_simple_code_execution_error(monkeypatch: pytest.MonkeyPatch):
    ...
    with pytest.raises(ToolInvokeError, match="boom"):
        list(
            simple_code.invoke(
                session=_ORM_SESSION, user_id="u", tool_parameters={"language": "python3", "code": "print(1)"}
            )
        )
...
def test_webscraper_empty_url():
    ...
    empty = list(webscraper.invoke(session=_ORM_SESSION, user_id="u", tool_parameters={"url": ""}))[0].message.text
...
def test_webscraper_fetch(monkeypatch: pytest.MonkeyPatch):
    ...
    full = list(webscraper.invoke(session=_ORM_SESSION, user_id="u", tool_parameters={"url": "https://example.com"}))[
        0
    ].message.text
    assert full == "page"
...
def test_webscraper_fetch_error(monkeypatch: pytest.MonkeyPatch):
    ...
    with pytest.raises(ToolInvokeError, match="boom"):
        list(webscraper.invoke(session=_ORM_SESSION, user_id="u", tool_parameters={"url": "https://example.com"}))


def test_asr_invalid_file():
    asr = _build_builtin_tool(ASRTool)
    file_obj = SimpleNamespace(type=FileType.DOCUMENT)
    invalid_file = list(asr.invoke(session=_ORM_SESSION, user_id="u", tool_parameters={"audio_file": file_obj}))[
        0
    ].message.text
...
def test_asr_valid_file_invocation(monkeypatch: pytest.MonkeyPatch):
    ...
    ok = list(
        asr.invoke(session=_ORM_SESSION, user_id="u", tool_parameters={"audio_file": audio_file, "model": "p#m"})
    )[0].message.text
...
def test_tts_invoke_returns_messages(monkeypatch: pytest.MonkeyPatch):
    ...
    messages = list(tts.invoke(session=_ORM_SESSION, user_id="u", tool_parameters={"model": "p#m", "text": "hello"}))
...
def test_tts_tool_raises_when_runtime_missing():
    ...
    tts.runtime = None
    with pytest.raises(ValueError, match="Runtime is required"):
        list(tts.invoke(session=_ORM_SESSION, user_id="u", tool_parameters={"model": "p#m", "text": "hello"}))
...
def _bind_sqlite_session(sqlite_engine: Engine):
    """Provide every built-in tool invocation with a real SQLite Session."""
    global _ORM_SESSION
