...
class CompletionWorkflowRunner(AppRunner):
    ...
    def run(
        ...
        app_record = self._get_app(app_id=app_config.app_id, tenant_id=app_config.tenant_id)
        ...
        runtime_workflow = build_runtime_completion_workflow(app_model=app_record, app_config=app_config)
        ...
        # Do not hold the request-scoped connection during provider streaming.
        db.session.close()
        for event in workflow_entry.run():
            adapter.handle_event(event)

    def _get_app(self, *, app_id: str, tenant_id: str) -> App:
        app_record = db.session.scalar(select(App).where(App.id == app_id, App.tenant_id == tenant_id))
        if not app_record:
